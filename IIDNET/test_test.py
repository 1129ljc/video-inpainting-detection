...
class IID_Dataset(Dataset):
    ...
    def load_item(self, idx):
        fname1 = self.train_files[idx]

        img = cv2.imread(fname1)
        img = cv2.resize(img, self.input_size)
        img = img.astype('float') / 255.
        return self.transform(img), fname1
...
class IID_Net(nn.Module):
    def __init__(self):
        ...
        self.cell1 = Block(64, 128, reps=3, stride=2, BatchNorm=BatchNorm, start_with_relu=False,
                           genotype=['01', '03', '00'])
        self.cell2 = Block(128, 256, reps=3, stride=2, BatchNorm=BatchNorm, start_with_relu=True, grow_first=True,
                           genotype=['02', '00', '00'])
        self.cell3 = Block(256, 256, reps=3, stride=1, BatchNorm=BatchNorm, start_with_relu=True, grow_first=True,
                           genotype=['03', '02', '00'])
        self.cell4 = Block(256, 256, reps=3, stride=1, dilation=2, BatchNorm=BatchNorm, start_with_relu=True,
                           grow_first=True, genotype=['01', '00', '01'])
        self.cell5 = Block(256, 256, reps=3, stride=1, dilation=2, BatchNorm=BatchNorm, start_with_relu=True,
                           grow_first=True, genotype=['00', '02', '00'])
        self.cell6 = Block(256, 256, reps=3, stride=1, dilation=2, BatchNorm=BatchNorm, start_with_relu=True,
                           grow_first=True, genotype=['00', '01', '00'])
        self.cell7 = Block(256, 256, reps=3, stride=1, dilation=2, BatchNorm=BatchNorm, start_with_relu=True,
                           grow_first=True, genotype=['02', '03', '02'])
        self.cell8 = Block(256, 256, reps=3, stride=1, dilation=2, BatchNorm=BatchNorm, start_with_relu=True,
                           grow_first=True, genotype=['03', '03', '00'])
        self.cell9 = Block(256, 256, reps=3, stride=1, dilation=2, BatchNorm=BatchNorm, start_with_relu=True,
                           grow_first=True, genotype=['02', '02', '00'])
        self.cell10 = Block(256, 256, reps=3, stride=1, dilation=2, BatchNorm=BatchNorm, start_with_relu=True,
                            grow_first=True, genotype=['00', '01', '03'])
...
def test(args):
    ...
    for idx, (image, image_path) in enumerate(test_loader):
        ...
        Ii, Mg = image.cuda(), image_path
        filename = str(image_path)[2:-3]
        Mo = giid_model.process(Ii)
        Ii, Mo = convert1(Ii), convert2(Mo)[0]
        print(Mo.shape)
        save0 = str(filename).split('/')
        save1, filename = save0[-2], save0[-1]
        if not os.path.exists(os.path.join(save, save1)):
            os.mkdir(os.path.join(save, save1))
        cv2.imwrite(os.path.join(save, save1, filename), Mo * 255)
