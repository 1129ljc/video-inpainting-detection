...
def frame2result(imagedir, maskdir, resultdir):
    ...
    for i in range(len(files)):
        ...
        image_cat = cv2.resize(image_cat, (h, w))
...
def test_image(input_a, input_b, input_c):
    ...
    input_arg_ext_out_tmp_path = os.path.join(input_arg_ext_tmp_dir, 'hp_fcn_own')
    ...
    algorithm_message = '该算法使用高通滤波卷积核作为预滤波模块，使用四层Renet_V2_Block作为编码器，两层上采样与卷积作为解码器'
    ...
    for i in range(len(images_detection)):
        ...
        image_feature_one = {'filepath': os.path.join(result_dir, image_locac_name),
                             'title': '视频逐帧修补定位示意图',
                             'comment': '该图展示待检测视频采样帧可疑的修补区域，输出为输入视频8个采样帧的二值掩膜图' }
